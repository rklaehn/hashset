...
package scala.collection.immutable7

import scala.annotation.unchecked.{ uncheckedVariance => uV }
import scala.collection.parallel.immutable.ParHashSet
import scala.collection.immutable.ListSet
import scala.collection.{immutable, SetLike, CustomParallelizable, AbstractSet}
import scala.collection.generic.{CanBuildFrom, ImmutableSetFactory, GenericCompanion, GenericSetTemplate}
...
object HashSet extends ImmutableSetFactory[HashSet] {
  ...
  @tailrec
  def get0[A](set:HashTrieSet[A], key: A, hash: Int, level: Int): Boolean = {
    ...
    val offset =
      if(set.bitmap == -1)
        ...
        return false
    set.elems(offset) match {
      case child:HashTrieSet[A] => get0(child, key, hash, level + 5)
      case child:HashSet1[A] => child.hash == hash && child.key == key
      case child:HashSetCollision1[A] => child.hash == hash && child.ks.contains(key)
    }
  }
  ...
  private[immutable7] class HashSetCollision1[A](private[HashSet] val hash: Int, val ks: ListSet[A])
    extends HashSet[A] {
  ...
  class HashTrieSet[A](private[immutable7] val bitmap: Int, private[collection] val elems: Array[HashSet[A]], private val size0: Int)
    extends HashSet[A] {
    ...
    override def contains(e: A): Boolean = HashSet.get0(this, e, computeHash(e), 0)
