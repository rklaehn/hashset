...
with Serializable
{
  import HashSet.{nullToEmpty, createBuffer}
  ...
  override def --(that: GenTraversableOnce[A]) = that match {
    case that: HashSet[A] =>
      // if that is also a HashSet, we can use diff, which is quicker
      this diff that
    case _ =>
      // in any other case, use the default implementation from Subtractable
      super.--(that)
  }

  override def filter(p: A => Boolean) = {
    val buffer = createBuffer[A](size)
    nullToEmpty(filter0(p, 0, buffer, 0))
  }

  override def filterNot(p: A => Boolean) = {
    val buffer = createBuffer[A](size)
    nullToEmpty(filter0(!p(_), 0, buffer, 0))
  }

  override def diff(that:GenSet[A]) = this filterNot that

  protected def filter0(p: A => Boolean, level: Int, buffer: Array[HashSet[A]], offset0: Int): HashSet[A] = null
...
object HashSet extends ImmutableSetFactory[HashSet] {
  ...
  class HashSet1[A](private[HashSet] val key: A, private[HashSet] val hash: Int) extends HashSet[A] {
    ...
    override protected def filter0(p: A => Boolean, level: Int, buffer: Array[HashSet[A]], offset0: Int): HashSet[A] =
  ...
  private[immutable5] class HashSetCollision1[A](private[HashSet] val hash: Int, val ks: ListSet[A])
    extends HashSet[A] {
  ...
  class HashTrieSet[A](private val bitmap: Int, private[collection] val elems: Array[HashSet[A]], private val size0: Int)
    ...
    override protected def filter0(p: A => Boolean, level: Int, buffer: Array[HashSet[A]], offset0: Int): HashSet[A] = {
      ...
      while (i < elems.length) {
        val result = elems(i).filter0(p, level + 5, buffer, offset)
        if (result ne null) {
          // add the result to the buffer
          buffer(offset) = result; offset += 1
          // add the result size
          rs += result.size
  ...
  /**
   * Create a buffer sufficient for building a tree of the given maximum size
   * @param size the maximum size of the tree to be built
   * @tparam A the element type
   * @return a sufficiently large array
   */
  @inline private def createBuffer[A](size:Int) : Array[HashSet[A]] = {
    new Array[HashSet[A]]((size + 6) min (32 * 7))
  }
