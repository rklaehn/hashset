...
object UnionDebug extends App {
  ...
  def testUnion[T](sizes: Seq[Int], offsets: Seq[Double], keyType: String, mkKey: Int => T) {
    for {
      ...
      val u = a union b
      require(hashCount == hashCount0, s"key.hashCode should not be called during intersect, $i $o but has been called ${hashCount - hashCount0} times. Key type $keyType.")
//      require(u == (a union scala.collection.mutable.HashSet(b.toSeq: _*)), s"Operation must still work for other sets!")
  ...
  testUnion(Seq(10), Seq(0.0), "collision", Collision.apply)
